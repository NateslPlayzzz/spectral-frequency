...
data modify storage sf:temp remnant.owner set from entity @s UUID

function sf:player/claim/ensure_remnant_macro with storage sf:temp remnant

# Remove orphaned visual or label components before reconstructing the complete assembly.
execute unless entity @s[tag=sf.remnant_present] run function sf:player/revive/cleanup_owner with storage sf:temp remnant
