# tool/totem/place.mcfunction — as @s at @s
scoreboard players set @s sf.tool_cd 10
# One ward at a time per area — refuse if standing in an existing ward
...
# Place the ward marker at the player's position, seed its life timer
summon marker ~ ~ ~ {Tags:["sf.ward","smithed.entity","smithed.strict"]}
execute as @e[type=marker,tag=sf.ward,sort=nearest,limit=1] run function sf:tool/totem/init_ward
# Consume the item + charge
...
title @s actionbar {"text":"a ward is set","color":"#FFC36B","italic":true}