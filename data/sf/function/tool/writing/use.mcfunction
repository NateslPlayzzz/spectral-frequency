...
# Begins a five-second automatic-writing exposure.
...
execute if score @s sf.writing_active matches 1.. run return run title @s actionbar {"text":"the page is already exposed","color":"dark_gray","italic":true}

scoreboard players set @s sf.writing_active 1
scoreboard players set @s sf.writing_timer 5

title @s actionbar {"text":"✎ WRITING DETECTOR  ·  page exposed","color":"#FFC36B","italic":true}
