...
# --- Standard-case authority and stale-entity rejection ---
execute unless data storage sf:case {state:"active"} as @e[type=minecraft:marker,tag=sf.ghost,tag=!sf.forgotten] run kill @s
execute unless data storage sf:case {state:"active"} as @e[type=minecraft:marker,tag=sf.ward] run kill @s
execute if data storage sf:case {state:"active"} run function sf:case/heartbeat
...
execute as @e[type=minecraft:marker,tag=sf.ghost,tag=!sf.forgotten,tag=sf.state.idle] at @s run function sf:ghost/idle/tick
...
execute as @e[type=minecraft:marker,tag=sf.ghost,tag=sf.state.warning] run scoreboard players remove @s sf.hunt_timer 1
execute as @e[type=minecraft:marker,tag=sf.ghost,tag=sf.state.hunt] run scoreboard players remove @s sf.hunt_timer 1
execute as @e[type=minecraft:marker,tag=sf.ghost,tag=sf.state.hunt,scores={sf.hunt_timer=..0}] run function sf:ghost/hunt/end
execute as @e[type=minecraft:marker,tag=sf.ghost,tag=sf.state.cooldown] run scoreboard players remove @s sf.cooldown_timer 1
execute as @e[type=minecraft:marker,tag=sf.ghost,tag=sf.state.cooldown,scores={sf.cooldown_timer=..0}] run function sf:ghost/idle
...
# --- Warding Totems ---
execute as @e[type=minecraft:marker,tag=sf.ward] at @s run function sf:tool/totem/tick

# --- Motion Sensors ---
execute as @e[type=minecraft:marker,tag=sf.sensor] at @s run function sf:tool/sensor/tick

# --- Attuned Bench ambience ---
execute as @e[type=minecraft:marker,tag=sf.bench] at @s run particle minecraft:wax_off ~ ~0.3 ~ 0.3 0.3 0.3 0.0 3
