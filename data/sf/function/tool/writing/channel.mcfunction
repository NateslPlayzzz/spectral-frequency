...
# Runs once per second while the page remains exposed.
...
execute if entity @s[gamemode=spectator] run return run function sf:tool/writing/break

# The Writing Detector must remain in the main hand throughout exposure.
...
execute if score @s sf.writing_timer matches 1.. run title @s actionbar [{"text":"✎ PAGE EXPOSED  ","color":"#FFC36B","bold":true},{"score":{"name":"@s","objective":"sf.writing_timer"},"color":"white"},{"text":"s","color":"gray"}]
