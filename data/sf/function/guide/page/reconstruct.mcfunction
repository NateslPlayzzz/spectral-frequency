...
tellraw @s [{"text": "Each distinct fragment you bind leaves behind a stable Memory pattern. Repeated manifestations still provide field experience and materials, but only the first record of each pattern advances Reconstruction.","color": "gray"}]

tellraw @s [{"text": "Your Field Radio compares new signals against your personal record and favors accessible patterns you have not yet remembered.","color": "#D8C8A0","italic": true}]

tellraw @s [{"text": "Carry an Echo Lens when opening a source to let the Radio resolve patterns beyond the standard registry.","color": "#C89BFF","italic": true}]
